...
def get_minio_client(lake_bucket: str = LAKE_BUCKET) -> Minio:
    """
    Returns a Minio client configured with the environment variables.
    """
    if not all([AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, lake_bucket, MINIO_HOST, MINIO_API_PORT]):
        raise ValueError("One or more required environment variables are not set.")
    config = {
        "minio_bucket": lake_bucket,
        "minio_endpoint": f"{MINIO_HOST}:{MINIO_API_PORT}",
